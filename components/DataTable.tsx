...
// Interface for the values returned by the useDataTable hook
interface DataTableHookValues<T> {
  paginatedItems: T[];
...
interface DataTableProps<T> {
  ...
  hook: DataTableHookValues<T>; // State and handlers are passed in via this prop
}
...
export const DataTable = <T extends Record<string, any>>({
  ...
  const getSortIcon = (key: keyof T) => {
    if (sortConfig.key !== key) {
      return <span className="opacity-20 group-hover:opacity-100 transition-opacity"><ArrowDown className="inline-block ml-1 h-4 w-4" /></span>;
    }
    ...
    <div className="w-full">
        <div className="mb-4">
            <div className="relative">
                ...
                <input
                    ...
                    className="w-full max-w-xs p-2 pl-10 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-brand-primary focus:border-brand-primary text-sm bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200"
                />
            </div>
        </div>
        ...
        {pageCount > 0 && (
             <div className="flex items-center justify-between mt-4 text-sm text-gray-600 dark:text-gray-400">
                <span>
                   Showing {Math.min(1 + (currentPage - 1) * 10, totalItems)}-{Math.min(currentPage * 10, totalItems)} of {totalItems}
                </span>
                {pageCount > 1 && (
