...
const LearnerSelector: React.FC<{
    allLearners: { email: string; name: string }[];
    selectedEmail: string;
    onSelect: (email: string) => void;
    label: string;
    placeholder: string;
}> = ({ allLearners, selectedEmail, onSelect, label, placeholder }) => {
        ...
        <div>
            <label htmlFor={`learner-input-${label}`} className="font-semibold text-sm">{label}:</label>
             <div className="relative mt-1">
                 <input
                    type="text"
                    id={`learner-input-${label}`}
                    list={`learners-list-${label}`}
                    value={inputValue}
                    onChange={handleInputChange}
                    placeholder={placeholder}
                    className="w-full p-2 pr-8 border rounded-md border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200"
                />
                <datalist id={`learners-list-${label}`}>
                    {allLearners.map(learner => (
                        <option key={learner.email} value={`${learner.name} (${learner.email})`} />
                    ))}
                </datalist>
                {inputValue && (
                    <button onClick={() => onSelect('')} className="absolute inset-y-0 right-0 flex items-center pr-2 text-gray-500"><X className="h-5 w-5" /></button>
                )}
            </div>
        </div>
    );
...
export const LearnerPerformance: React.FC<LearnerPerformanceProps> = ({ allData, selectedEmail, onLearnerSelect }) => {
    const [selectedEmail2, setSelectedEmail2] = useState('');

    const uniqueLearners = useMemo(() => {
        const learnerMap = new Map<string, string>(); // Map<email, name>
        allData.forEach(d => {
            if (d.email && !learnerMap.has(d.email)) {
                learnerMap.set(d.email, d.traineeName);
            }
        });
    ...
    const learnerChartData = useMemo(() => {
        ...
        return allDates.map(dateMs => {
            ...
            return {
                ...
                learner1Score: record1?.postAssessmentScore,
                learner1Course: record1?.courseTitle,
                learner2Score: record2?.postAssessmentScore,
                learner2Course: record2?.courseTitle,
            };
        });
    ...
    const completedCoursesData = useMemo(() => {
        ...
        return allData
            .filter(d => d.email === selectedEmail && d.postAssessmentScore > 0)
            .map(d => ({
                courseTitle: d.courseTitle,
                completionDate: d.completionDate,
                postAssessmentScore: d.postAssessmentScore
            }));
    }, [selectedEmail, allData]);

    const courseTableHook = useDataTable<CompletedCourseData>(completedCoursesData, 'completionDate', 5);

    const handleExport = () => {
        if (courseTableHook.sortedItems.length === 0) {
            alert("No data to export.");
            return;
        }
        const dataToExport = courseTableHook.sortedItems.map(item => ({
            'Course Title': item.courseTitle,
            'Completion Date': item.completionDate,
            'Score (%)': item.postAssessmentScore,
        }));
    ...
    const learnerAverageScore = useMemo(() => {
        ...
        const totalScore = completedCoursesData.reduce((acc, course) => acc + course.postAssessmentScore, 0);
        return totalScore / completedCoursesData.length;
    ...
    const courseTableColumns: Column<CompletedCourseData>[] = [
    ...
    const SortControls = () => {
        ...
        const sortOptions: { label: string; key: keyof CompletedCourseData }[] = [
            { label: 'Course', key: 'courseTitle' }, { label: 'Date', key: 'completionDate' }, { label: 'Score', key: 'postAssessmentScore' },
        ];
        return (
            <div className="flex items-center space-x-2">
                ...
                {sortOptions.map(opt => {
                    ...
                    return (
                        <button key={opt.key as string} onClick={() => requestSort(opt.key)}
                            className={`flex items-center px-3 py-1.5 text-xs rounded-md font-medium transition-colors ${ isActive ? 'bg-brand-primary text-white shadow' : 'bg-gray-200 dark:bg-gray-600 hover:bg-gray-300 dark:hover:bg-gray-500'}`}>
                            {opt.label}
                            {isActive && (sortConfig.direction === 'ascending' ? <ArrowUp className="h-3 w-3 ml-1.5" /> : <ArrowDown className="h-3 w-3 ml-1.5" />)}
                        </button>
                    );
                })}
            </div>
            ...
            return (
                <div className="bg-white dark:bg-gray-800 p-3 border dark:border-gray-600 rounded-lg shadow-xl text-sm">
                    ...
                    {payload.map((p: any, index: number) => (
                        p.payload[`learner${index + 1}Score`] && (
                            <div key={p.dataKey} style={{ color: p.color }}>
                                <p className="font-semibold">{p.name}: {p.value}%</p>
                                <p className="text-xs text-gray-500 dark:text-gray-400 pl-2">Course: {p.payload[`learner${index + 1}Course`]}</p>
                            </div>
                        )
                    ))}
                </div>
                ...
                <div className="flex-grow" style={{ minHeight: '300px' }}>
                    {(selectedEmail || selectedEmail2) && learnerChartData.length > 0 ? (
                        ...
                        <div className="flex items-center justify-center h-full text-gray-500 dark:text-gray-400">
                           {selectedEmail ? 'No data for this learner.' : 'Please select at least one learner to view their performance.'}
                        </div>
                    )}
                </div>
                ...
                {selectedEmail && completedCoursesData.length > 0 && (
                    <div className="mt-8">
                        <h4 className="text-md font-bold text-brand-dark dark:text-white mb-4 flex items-center gap-x-3">
                           ...
                           {isTopPerformer && (
                               <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-teal-100 text-teal-800 dark:bg-teal-900 dark:text-teal-200 ring-1 ring-inset ring-teal-200 dark:ring-teal-800">
                                   <Star className="h-3 w-3 mr-1.5 text-yellow-500" fill="currentColor" />
                                   Top Performer
                               </span>
                           )}
                        </h4>
                        <div className="flex justify-between items-center mb-3">
                            ...
                            <button
                                ...
                                className="flex items-center px-3 py-1.5 text-sm font-medium text-gray-600 dark:text-gray-300 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm hover:bg-gray-50 dark:hover:bg-gray-600"
                                aria-label="Export completed courses to CSV"
                            >
                        ...
                        <DataTable
                            hook={courseTableHook}
                            columns={courseTableColumns}
                            rowClassName={() => isTopPerformer ? 'bg-teal-50 dark:bg-teal-950 hover:bg-teal-100 dark:hover:bg-teal-900' : '' }
                        />
                    </div>
                )}
            </div>
