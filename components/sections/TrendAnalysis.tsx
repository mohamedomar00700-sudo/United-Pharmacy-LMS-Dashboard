...
const calculateMovingAverage = (data: { avgCompletion: number }[], windowSize: number) => {
    if (!data || data.length < windowSize) return data.map(d => ({ ...d, movingAverage: null }));

    return data.map((_, index, arr) => {
        if (index < windowSize - 1) {
            return { ...arr[index], movingAverage: null };
        }
...
export const TrendAnalysis: React.FC<TrendAnalysisProps> = ({ data }) => {
    ...
    const monthlyData = useMemo(() => {
        ...
        data.forEach(item => {
            ...
            if (!trends[month]) {
                trends[month] = { totalCompletion: 0, count: 0, totalHours: 0 };
            }
