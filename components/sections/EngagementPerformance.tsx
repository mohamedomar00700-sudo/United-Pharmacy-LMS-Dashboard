...
const CustomTooltip = ({ active, payload }: any) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-white dark:bg-gray-800 p-2 border dark:border-gray-600 rounded shadow-lg text-sm">
        <p className="font-bold text-gray-800 dark:text-gray-100">{data.name}</p>
        <p className="text-gray-700 dark:text-gray-300">{`Engagement (Hours): ${data.x.toFixed(1)}`}</p>
        <p className="text-gray-700 dark:text-gray-300">{`Performance (Score): ${data.y.toFixed(1)}%`}</p>
      </div>
    );
  }
  return null;
...
export const EngagementPerformance: React.FC<EngagementPerformanceProps> = ({ data }) => {
    ...
    const { scatterData, avgEngagement, avgPerformance } = useMemo(() => {
        ...
        const processedData = learners.map(learner => {
            ...
            return {
                name: learner,
                x: totalHours, // Engagement: Training Hours
                y: avgScore,   // Performance: Post-Assessment Score
                z: learnerRecords.length * 50, // bubble size based on course count
            };
        }).filter(d => !isNaN(d.y));

        if (processedData.length === 0) {
            return { scatterData: [], avgEngagement: 0, avgPerformance: 0 };
        }
    ...
    const QuadrantLabel: React.FC<{x: number, y: number, label: string, color: string}> = ({ x, y, label, color }) => (
         <Label 
            value={label} 
            ...
            content={(props) => {
                ...
                const { x: vx, y: vy } = viewBox;
                return (
                    <g>
                        <text x={x} y={y} fill={color} fontSize="12" fontWeight="bold" textAnchor="middle">
                           {label}
                        </text>
                    </g>
                )
    ...
    return (
        <DashboardCard title="Engagement vs. Performance Correlation">
            <div style={{ width: '100%', height: 450 }} className="relative">
                 <div className="absolute top-[10%] left-[55%] text-center text-xs text-success font-bold opacity-80">
                    <p>High Achievers</p>
                    <p>(High Perf / High Eng.)</p>
                </div>
                <div className="absolute bottom-[15%] left-[55%] text-center text-xs text-warning font-bold opacity-80">
                    <p>Need Support</p>
                    <p>(Low Perf / High Eng.)</p>
                </div>
                 <div className="absolute top-[10%] right-[55%] text-center text-xs text-brand-primary font-bold opacity-80">
                    <p>Efficient</p>
                    <p>(High Perf / Low Eng.)</p>
                </div>
                 <div className="absolute bottom-[15%] right-[55%] text-center text-xs text-danger font-bold opacity-80">
                    <p>Need Motivation</p>
                    <p>(Low Perf / Low Eng.)</p>
                </div>
                ...
                <ResponsiveContainer>
                    <ScatterChart margin={{ top: 40, right: 40, bottom: 40, left: 40 }}>
                        ...
                        <ReferenceLine y={avgPerformance} stroke={tickColor} strokeDasharray="4 4">
                            <Label value="Avg. Perf." position="right" fill={tickColor} fontSize={10}/>
                        </ReferenceLine>
                        <ReferenceLine x={avgEngagement} stroke={tickColor} strokeDasharray="4 4">
                             <Label value="Avg. Eng." position="top" fill={tickColor} fontSize={10}/>
                        </ReferenceLine>
