...
export const Overview = ({ data, allData, onNavigate }: OverviewProps) => {
    const getTrendData = (records: TrainingRecord[], key: keyof TrainingRecord, days = 30) => {
        const sorted = [...records].sort((a,b) => a.completionDate.getTime() - b.completionDate.getTime());
        if (sorted.length < 2) return [];
        const trend = sorted.slice(-days).map(d => ({ value: d[key] as number }));
    ...
    const stats = useMemo(() => {
        ...
        const sparklines = {
            ...
            improvement: getTrendData(data.map(d => ({...d, improvement: d.preAssessmentScore > 0 ? ((d.postAssessmentScore - d.preAssessmentScore)/d.preAssessmentScore)*100 : 0})), 'improvement' as any)
        };
        ...
        return { 
            ...
            avgCompletionChange: { text: changeText, color: changeColor }, 
            sparklines 
        };
    }, [data, companyAvgs]);
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <KpiCard title="Total Learners" value={stats.totalLearners} icon={<Users className="text-brand-primary" />} tooltip="Total unique trainees. Click to view leaderboards." onClick={() => onNavigate('leaderboard')} sparklineData={stats.sparklines.learners} />
            ...
            <KpiCard title="Active vs Inactive Learners" value={`${stats.activeLearners} / ${stats.inactiveLearners}`} icon={<UserX className="text-warning" />} tooltip="Active learners have a completion rate > 0%. Click to view at-risk trainees." onClick={() => onNavigate('actionable-insights')} />
